"""Local document loading, chunking, indexing, and relevance ranking."""
...
class KnowledgeChunk:
    """Represent one indexed document or document fragment."""
...
class RetrievedChunk:
    """Represent one ranked retrieval result with its relevance score."""
...
class LocalKnowledgeBase:
    """Provide deterministic local retrieval over API chunks or complete Scene files."""
    ...
    @classmethod
    def from_document_directory(
        cls, directory: str | Path, *, source_prefix: str = ""
    ) -> "LocalKnowledgeBase":
        """Index one full document as one searchable unit.

        API knowledge uses ``from_directory`` and is deliberately chunked. Scene
        selection uses this constructor so ranking happens between documents and
        the selected result still contains the complete source document.
        """
        ...
        for path in sorted(root.rglob("*")):
            ...
            documents.append(
                KnowledgeChunk(
                    source=f"{source_prefix}{path.relative_to(root)}",
                    content=content,
                    heading=_document_heading(content, path),
                    kind="scene",
                )
            )
...
def _document_heading(content: str, path: Path) -> str:
    if path.suffix.lower() == ".md":
