"""Perform syntax, import, dangerous-call, and output-path validation on code."""
...
def validate_code(
    code: str,
    task_spec: TaskSpec,
    api_context: Iterable[dict[str, Any]] = (),
) -> CodeValidation:
    errors: list[str] = []
    if not code.strip():
    ...
    allowed = _allowed_import_roots(task_spec) | _documented_import_roots(api_context)
...
def _documented_import_roots(context: Iterable[dict[str, Any]]) -> set[str]:
    roots: set[str] = set()
    for item in context:
...
def _absolute_write_path(
    ...
    if call_name in {"open", "builtins.open"} and node.args:
