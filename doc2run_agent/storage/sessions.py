"""Versioned, filesystem-backed session persistence and reset handling."""
...
from .files import atomic_json_write, atomic_text_write
...
class FileSessionStore:
    """Load, save, version, archive, and write artifacts for local sessions."""
    ...
    def has_session(self, session_id: str) -> bool:
        """Return whether a persisted session exists without creating it."""

        return self._session_file(session_id).is_file()

    def list_session_ids(self) -> list[str]:
        """Return persisted session IDs, excluding archived sessions."""

        return sorted(
            path.name
            for path in self.base_directory.iterdir()
            if path.is_dir()
            and path.name != "archives"
    ...
    def snapshot_confirmed_spec(self, record: SessionRecord) -> TaskSpec:
        ...
        if not record.draft_plan.strip():
            raise ValueError("Scenario Plan is not ready for confirmation")
        ...
        record.confirmed_plan = record.draft_plan.strip()
        record.draft_spec = snapshot.model_copy(deep=True)
        record.status = "confirmed"
        atomic_text_write(
            self.session_directory(record.session_id) / "planning" / "confirmed_plan.md",
            record.confirmed_plan + "\n",
        )
        self.save(record)
        return snapshot
